> Source: https://developers.weixin.qq.com/minigame/dev/api/chattool/wx.notifyGroupMembers.html
...
|  | 合法值 | 说明 |
| participate | 请参与 |
| complete | 请完成 |  success function  否 接口调用成功的回调函数  fail function  否 接口调用失败的回调函数  complete function  否 接口调用结束的回调函数（调用成功、失败都会执行） ## # 示例代码
```javascript
