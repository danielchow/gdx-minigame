> Source: https://developers.weixin.qq.com/minigame/dev/api/media/voip/wx.joinVoIPChat.html
...
**[用户授权](../../../guide/base-ability/authorize.html)**：需要 scope.record
...
## 功能描述
加入 (创建) 实时语音通话，更多信息可见 [实时语音指南](../../../guide/open-ability/voip-chat.html)。调用前需要用户授权 `scope.record`，若房间类型为视频房间需要用户授权 `scope.camera`。
## 参数
