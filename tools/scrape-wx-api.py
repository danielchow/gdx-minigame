#!/usr/bin/env python3
"""
scrape-wx-api.py — Fetch all WeChat minigame wx.* API docs from the official docs site.

Discovers all API URLs from the sidebar, fetches each page in parallel,
extracts the content div, converts to clean markdown, and saves to disk.

Usage:
    python3 tools/scrape-wx-api.py                              # fetch all APIs
    python3 tools/scrape-wx-api.py --dry-run                    # just list URLs
    python3 tools/scrape-wx-api.py --category base              # only "base" category
    python3 tools/scrape-wx-api.py --category network/websocket # specific subcategory
    python3 tools/scrape-wx-api.py --concurrency 10             # parallel downloads
    python3 tools/scrape-wx-api.py --output ./my-dir            # custom output dir
    python3 tools/scrape-wx-api.py --retry 3                    # retries on failure

Output: references/wx-api-docs/<category>/<...>/<name>.md
"""
...
INDEX_URL = f"{BASE_URL}/minigame/dev/api/base/system/wx.getSystemInfoSync.html"


def fetch_text(url: str, timeout: int = 20) -> str:
    """Fetch URL and return decoded text."""
    # URL-encode any non-ASCII characters in the path
    parsed = urlparse(url)
...
def extract_api_urls(index_html: str) -> list[str]:
    """Extract all API doc URLs from the sidebar of any minigame API page."""
    # The sidebar contains all API links as href="/minigame/dev/api/...html"
    pattern = r'href="(/minigame/dev/api/[^"]+\.html)"'
...
def fetch_one(rel_path: str, output_dir: str, retries: int = 2) -> tuple[str, str, int]:
    ...
    # Compute output path
    api_path = rel_path.removeprefix("/minigame/dev/api/")
...
def main():
    parser = argparse.ArgumentParser(description="Scrape WeChat minigame API docs")
    parser.add_argument("--dry-run", action="store_true", help="List URLs without fetching")
    parser.add_argument("--category", default="", help="Filter to a specific category (e.g. 'base', 'network/websocket')")
    parser.add_argument("--concurrency", type=int, default=8, help="Parallel downloads (default: 8)")
    parser.add_argument("--output", default="", help="Output directory (default: references/wx-api-docs)")
    ...
    output_dir = args.output or str(project_dir / "references" / "wx-api-docs")

    print(f"==> Step 1: Discovering API URLs from sidebar...")
    ...
    # Filter by category
    if args.category:
        prefix = f"/minigame/dev/api/{args.category.strip('/')}/"
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as pool:
        ...
        for future in as_completed(futures):
            ...
            api_name = rel_path.removeprefix("/minigame/dev/api/")
            if status == "ok":
                ok += 1
